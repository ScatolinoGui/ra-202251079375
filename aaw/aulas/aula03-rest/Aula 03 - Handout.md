...
> **Nome:** Guilherme Pereira Scatolino
...
Usa POST para consulta, tem verbo na rota e retorna tudo sem paginação.

2. Seu redesenho (método + rota + status codes):

GET /api/v1/alunos, com 200 OK e 400 Bad Request se a paginação for inválida.
...
Usa GET para apagar, tem verbo na rota, id na query e sempre retorna 200.

2. Seu redesenho (método + rota + status codes):

DELETE /api/v1/alunos/7, com 204 No Content ao excluir e 404 Not Found se não existir.
...
Deveria retornar 201 Created, com Location, e não só 200 OK.

2. Seu redesenho (método + rota + status codes):

POST /api/v1/alunos, com 201 Created e 400 Bad Request se o body estiver inválido.
...
Aninhamento exagerado, rota muito longa e muitos ids desnecessários.

2. Seu redesenho (método + rota + status codes):

GET /api/v1/alunos/{id}/matriculas/{matriculaId}, com 200 OK e 404 Not Found.
...
Retorna 200 mesmo com erro, manda HTML e quebra o contrato JSON.

2. Seu redesenho (método + rota + status codes):

GET /api/v1/alunos/7/matriculas, com 200 OK ou 404 Not Found em JSON/ProblemDetails.
...
Usa PUT para atualização parcial, manda tudo na query e tem verbo na rota.

2. Seu redesenho (método + rota + status codes):

PATCH /api/v1/alunos/7/disciplinas/12/nota-parcial, com body JSON, 200 ou 204, e 404 se não existir.
...
Falta versionamento na API. O ideal é usar /api/v1/... e depois /api/v2/... para manter a versão antiga funcionando.
